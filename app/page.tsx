...
const DEFAULT_LISTINGS = [
  ...
  { id: "12", title: "Executive Ergonomic Office Chair - Lumbar Support", price: 75000, cat: "furniture", loc: "Lagos", images: [], desc: "High-quality ergonomic office chair with adjustable lumbar support, headrest, and armrests. Breathable mesh back, cushioned seat. Gas lift height adjustment. 360° swivel with smooth-rolling casters. Max weight capacity: 150kg. Assembly included for Lagos buyers.", seller: "FurnitureMart NG", sellerId: "seller-12", sellerPhone: "+2349021234567", verified: true, promoted: false, views: 78, saves: 5, rating: "4.2", createdAt: new Date(Date.now() - 168 * 3600000), status: "active" },
  { id: "13", title: "Brazilian Straight Hair 22\" - 100% Human Hair", price: 38000, cat: "haircare", loc: "Lagos", images: [], desc: "Premium quality 100% virgin Brazilian straight hair, 22 inches. Can be bleached, dyed, curled, and heat-styled. Minimal shedding, no tangling. Lasts 2+ years with proper care. Available lengths: 14\"-30\". Wholesale prices for hair vendors. Same-day delivery in Lagos.", seller: "HairByAda Lagos", sellerId: "seller-13", sellerPhone: "+2349031234567", verified: false, promoted: false, views: 213, saves: 19, rating: "4.4", createdAt: new Date(Date.now() - 10 * 3600000), status: "active" },
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (userListings) {
      ...
      setAllListings(prev => [...parsed.map((l: any) => ({ ...l, createdAt: new Date(l.createdAt) })), ...prev]);
    }
  }, []);
  ...
  const toggleSave = (id: string) => {
    setSavedIds((prev) => {
      ...
      else { next.add(id); toast("Saved to favorites ❤️"); }
      return next;
    });
  };
  ...
  const myListings = useMemo(() => {
    return allListings.filter(l => l.sellerId === (localUser?.id || "current-user"));
  }, [allListings, localUser]);

  const promoted = allListings.filter((l) => l.promoted && l.status === "active");

  const handleLogin = (role: string, data: any) => {
    localStorage.setItem("qsh_role", role);
    const user = { id: data.id || "current-user", role, name: data.name || "Quick User", email: data.email, avatar: data.avatar };
    setLocalUser(user);
    setView("app");
    ...
    toast.success(`Welcome, ${data.name || "there"}! You're logged in as ${role}.`);
  };

  const handleSwitchRole = (role: string) => {
    localStorage.setItem("qsh_role", role);
    setLocalUser((prev: any) => ({ ...prev, role }));
    ...
    const labels: Record<string, string> = { buyer: "Buyer", seller: "Seller", both: "Buyer & Seller", driver: "Driver", admin: "Admin" };
  ...
  const handlePostListing = (form: any) => {
    ...
    // Persist user listings
    const existing = JSON.parse(localStorage.getItem("qsh_user_listings") || "[]");
    localStorage.setItem("qsh_user_listings", JSON.stringify([newListing, ...existing]));

    toast.success("Ad posted successfully! It's now live.");
    setPage(localUser?.role === "seller" || localUser?.role === "both" ? "my-listings" : "home");
  };

  const handleDeleteListing = (id: string) => {
    setAllListings(prev => prev.map(l => l.id === id ? { ...l, status: "deleted" as any } : l));
    const existing = JSON.parse(localStorage.getItem("qsh_user_listings") || "[]");
    localStorage.setItem("qsh_user_listings", JSON.stringify(existing.filter((l: any) => l.id !== id)));
  ...
  const handleBoostListing = (id: string) => {
    setAllListings(prev => prev.map(l => l.id === id ? { ...l, promoted: true } : l));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`text-center py-1.5 text-xs font-semibold ${
        ...
        {role === "both" && "🔄 Buyer & Seller — Buy and sell on the platform"}
        {role === "driver" && "🚚 Driver Mode — Accept and deliver orders"}
        {role === "admin" && "⚙️ Admin Mode — Platform management"}
      </div>

      <Header search={search} onSearchChange={(v) => { setSearch(v); setPage("home"); }} page={page} onNavigate={setPage} userRole={role} userName={localUser?.name} userAvatar={localUser?.avatar} onSwitchRole={handleSwitchRole} onLogout={handleLogout} />
      ...
      <main className="max-w-[1280px] mx-auto px-4 sm:px-5 py-5 pb-28">
        ...
        {page === "my-listings" && (
          <SellerDashboard
            listings={myListings}
            onPostAd={() => { setEditingListing(null); setPage("post"); }}
          ...
          <>
            {isSeller && (
              <div className="bg-brand-yellow-bg border border-brand-yellow/30 rounded-2xl p-5 mb-5 flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setEditingListing(null); setPage("post"); }} className="px-4 py-2 bg-brand-yellow text-gray-900 rounded-lg font-display font-bold text-sm hover:bg-brand-yellow-dark transition-all">+ Post Ad</button>
                  <button onClick={() => setPage("my-listings")} className="px-4 py-2 bg-white text-gray-700 rounded-lg font-medium text-sm border border-gray-200 hover:bg-gray-50 transition-all">My Listings ({myListings.filter(l => l.status === "active").length})</button>
                </div>
              </div>
            )}
            ...
            {isDriver && (
              <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5 mb-5 flex items-center justify-between flex-wrap gap-3">
                ...
                <button onClick={() => setPage(driverVerified ? "deliveries" : "driver-verify")} className="px-4 py-2 bg-orange-500 text-white rounded-lg font-display font-bold text-sm hover:bg-orange-600 transition-all">View Jobs</button>
              </div>
            )}

            {!search && !selectedCat && <Hero onPostAd={() => { setEditingListing(null); setPage("post"); }} />}
            <CategoryChips selected={selectedCat} onSelect={(c) => setSelectedCat(selectedCat === c ? null : c)} />

            <div className="flex gap-2.5 mb-5 flex-wrap">
              <select className="px-3.5 py-2 border-[1.5px] border-gray-200 rounded-lg text-sm text-gray-700 bg-white outline-none focus:border-brand-blue" value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)}>
                <option value="">All Locations</option>
                {LOCATIONS.map((l) => <option key={l} value={l}>{l}</option>)}
              </select>
              <select className="px-3.5 py-2 border-[1.5px] border-gray-200 rounded-lg text-sm text-gray-700 bg-white outline-none focus:border-brand-blue" value={priceFilter} onChange={(e) => setPriceFilter(e.target.value)}>
            ...
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-xl font-bold">{search ? `Results for "${search}"` : selectedCat ? CATEGORIES.find((c) => c.id === selectedCat)?.name : "Latest Listings"}</h2>
        ...
        {page === "post" && <CreateListingForm onSubmit={handlePostListing} onCancel={() => setPage(isSeller ? "my-listings" : "home")} editing={editingListing} />}
        {page === "chat" && <ChatView />}
        ...
        {page === "saved" && (
          <>
            ...
            {savedIds.size > 0 ? (
              <ListingGrid listings={allListings.filter((l) => savedIds.has(l.id) && l.status === "active")} savedIds={savedIds} onSelect={setSelectedListing} onSave={toggleSave} />
            ) : (
              <div className="text-center py-16">
        ...
        {page === "profile" && <ProfileView user={localUser} listings={myListings.filter(l => l.status === "active")} savedIds={savedIds} onSelect={setSelectedListing} onSave={toggleSave} />}
      </main>

      {selectedListing && (
        <ListingDetail listing={selectedListing} onClose={() => setSelectedListing(null)} onChat={() => { setPage("chat"); setSelectedListing(null); toast("Chat opened with seller"); }} />
      )}
    </div>
  );
}
